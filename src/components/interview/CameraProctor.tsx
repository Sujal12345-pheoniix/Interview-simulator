...
export function CameraProctor() {
    ...
    const startCamera = async () => {
      try {
        stream = await navigator.mediaDevices.getUserMedia({ 
          ...
          audio: false // audio handled separately for speech recognition
        });
        ...
        const updateTracking = () => {
          if (!isMinimized) {
            // Generate random tracking points on the face area
            const points = [];
            for (let i = 0; i < 5; i++) {
              points.push({
          ...
          animationId = setTimeout(() => requestAnimationFrame(updateTracking), 500);
        };
        updateTracking();
  ...
  return (
    <div className={`relative overflow-hidden transition-all duration-500 ease-in-out border border-gray-800 bg-black shadow-2xl ${
      ...
      {!isMinimized && isAnalyzing && (
        <>
          ...
          {/* Facial/Posture Tracking Points */}
